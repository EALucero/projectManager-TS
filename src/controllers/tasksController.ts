...
export const taskStore = async (req: Request, res: Response) => {
  try {
    ...
    console.log(error);
    errorResponse(res, error, "TASK-STORE");
  }
};
...
export const taskUpdate = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      throw createError(
        400,
        "El nombre, la descripción, la prioridad y el cliente son datos obligatorios"
      );
    ...
    const task = await Task.findById(id).populate<{ project: IProject }>(
      "project"
    );
    if (!task) throw createError(404, "Tarea no encontrada");

    if (
      task.project.createdBy.toString() !==
      (req.user._id as Types.ObjectId).toString()
    )
      throw createError(403, "No estas autorizado!");
    ...
    await task.save();
    const taskUpadated = await Task.findById(id)
      .populate<{ project: IProject }>("project")
      .populate<{ user: User }>("assigned");
    ...
    console.log(error);
    errorResponse(res, error, "TASK-UPDATE");
  }
};
...
export const taskRemove = async (req: Request, res: Response) => {
  try {
    ...
    console.log(error);
    errorResponse(res, error, "TASK-DELETE");
  }
};
...
export const taskChangeState = async (req: Request, res: Response) => {
  try {
    ...
    console.log(error);
    errorResponse(res, error, "TASK-CHANGE-STATE");
  }
};
